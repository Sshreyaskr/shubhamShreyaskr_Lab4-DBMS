...
select count(cus_gender) as No_of_customers,cus_gender from customer where
cus_id in (select cus_id from orders
where ord_amount>=3000) group by cus_gender;

/*4) Display all the orders along with product name ordered by a customer having Customer_Id=2*/

select product.pro_name,orders.* from orders,product,supplier_pricing
...
select supplier.* from supplier where supp_id in (select supp_id from supplier_pricing
group by supp_id
...
(select pro_id from supplier_pricing where pricing_id 
...
(select orders.pricing_id,rating.ord_id,rating.rat_ratstars from orders inner join 
rating on rating.ord_id=orders.ord_id) as test on test.pricing_id=supplier_pricing.pricing_id)
